...
When TLS terminates at a reverse proxy, set `MT_ADMIN_BEHIND_HTTPS_PROXY=true`, keep `MT_ADMIN_ALLOW_INSECURE_HTTP=false`, and list every browser-facing origin in `MT_ADMIN_PUBLIC_ORIGINS`. The allowlist accepts at most 16 complete HTTPS origins and is intentionally independent of the internal source `Host`. The server ignores `Forwarded`, `X-Forwarded-Host`, and `X-Forwarded-Proto`; the proxy must overwrite client-supplied forwarding metadata and the source port must be reachable only from the trusted proxy.
