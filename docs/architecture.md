...
cmd/mt-server
    -> internal/app                    组合根与动态运行时
       -> internal/platform/*         状态、鉴权、HTTP、健康与设备位置
...
网页提交完整配置时必须通过同源、CSRF 和全局限速校验。QWeather 实时验证必须带一次性浏览器测试位置；验证成功后才以 `0600` 权限原子创建状态。临时位置不持久化，也不进入设备天气缓存。
...
纬度、经度和定位服务标识必填；城市、地区、国家和时区可选。服务器不读取连接来源 IP、`X-Forwarded-For`、Cloudflare 位置头或查询参数，也不调用 QWeather GeoAPI 或其他定位服务。

设备令牌持有者被允许声明任意位置。每个设备的位置变更令牌桶容量为 4，每 5 分钟恢复 1 次；同一网格不计数，超限返回 `429`。内存 LRU 最多保存 64 个位置，天气数据按网格、种类、语言和单位隔离；显示元数据始终来自当前请求，不跨设备缓存。
...
- 新天气供应商实现 `weather.Provider`，设备来源归属随供应商变化。
- 新设备定位服务只需映射固定请求头，不进入服务端适配层。
