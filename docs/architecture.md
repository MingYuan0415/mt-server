...
活动 QWeather Provider 最多执行 8 个并发上游请求，等待并发槽时响应请求取消。管理连接测试全局单并发且每分钟最多 6 次。网络错误和 5xx 立即重试一次；随后缓存按 5 秒起步、带 20% 抖动的指数退避冷却，最长 5 分钟。401/403 打开 15 分钟凭据熔断，429 按 `Retry-After` 建立最长 15 分钟的账户阻断，400 只冷却当前缓存键。熔断期间健康检查不可用，但未超过上限的陈旧缓存仍可返回。
